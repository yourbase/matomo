...
class SearchEngineKeywordsPerformance extends \Piwik\Plugin
{
    ...
    public function manipulateApiRequests(
        ...
        if ('Referrers' === $pluginName && 'getKeywordsFromSearchEngineId' === $methodName) {
            ...
            if ($row) {
                ...
                # If requesting a Yandex subtable and import is configured
                elseif (strpos($label, 'Yandex') !== false && $report->isYandexEnabled()) {
                    ...
                    $returnedValue = Request::processRequest('SearchEngineKeywordsPerformance.getKeywordsYandex',
                        ['idSubtable' => null, 'segment' => null, 'filter_sort_column' => 'nb_clicks', 'filter_limit' => null]);
                }
            }
    ...
    public function manipulateSearchEnginesReport(&$returnedValue, $finalParameters)
    {
        ...
        $returnedValue->filter(
            ...
            [
                'label',
                function ($label) use ($bingEnabled, $googleEnabled, $yandexEnabled) {
                    if ($bingEnabled && (strpos($label, 'Yahoo') !== false || strpos($label, 'Bing') !== false)) {
                        return 'Bing & Yahoo!';
                    } else if ($googleEnabled && strpos($label, 'Google') !== false) {
                        return 'Google';
                    } else if ($yandexEnabled && strpos($label, 'Yandex') !== false) {
                        return 'Yandex';
                    }
                    return $label;
                }

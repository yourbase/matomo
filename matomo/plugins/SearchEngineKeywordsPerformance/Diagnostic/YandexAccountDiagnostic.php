...
class YandexAccountDiagnostic implements Diagnostic
{
    ...
    public function execute()
    {
        ...
        $resultAccounts = new DiagnosticResult(
            Yandex::getInstance()->getName() . ' - ' . $this->translator->translate('SearchEngineKeywordsPerformance_ConfiguredAccounts')
        );
        ...
        foreach ($accounts as $id => $account) {
            ...
            } else if ($oauthDaysAgo >= 150) {
                $item = new DiagnosticResultItem(
                    DiagnosticResult::STATUS_WARNING,
                    $userInfo['name'] . ': ' .
                    $this->translator->translate('SearchEngineKeywordsPerformance_OAuthAccessWillTimeOutSoon', 180 - $oauthDaysAgo)
                );
            } else {
                $item = new DiagnosticResultItem(
                    DiagnosticResult::STATUS_OK,
                    $userInfo['name'] . ': ' .
                    $this->translator->translate('SearchEngineKeywordsPerformance_YandexAccountOk')
                );
            }
        ...
        $resultMeasurables = new DiagnosticResult(
            Yandex::getInstance()->getName() . ' - ' . $this->translator->translate('SearchEngineKeywordsPerformance_MeasurableConfig')
        );

...
class Google
{
    ...
    public function importAllAvailableData($limitKeywordDates = null)
    {
        ...
        if (is_string($limitKeywordDates) && strlen($limitKeywordDates) == 10) {
            ...
            if ($limitKeywordDates > 0) {
                $limitKeywordDates += 5; // always import 5 days more in the past, to ensure that non final data is imported again.
    ...
    protected function importKeywordsForListOfDates($datesToImport)
    {
        foreach ($datesToImport as $date) {
            foreach (self::$typesToImport as $type) {
                $this->importKeywordsIfNecessary(
                    $this->accountId,
    ...
    public function importKeywordsIfNecessary($accountId, $url, $date, $type, $force = false)
    {
        ...
        if ($keywordData) {
            ...
            if ($isTemporary === true) {
                Log::info('[SearchEngineKeywordsPerformance] Forcing reimport Google keywords for ' . $url . ' as imported data was not final.');
                $force = true;
            }
        }

        if ($keywordData && !$force) {
            Log::info('[SearchEngineKeywordsPerformance] Skipping import of Google keywords for ' . $date . ' and ' . $url . ' as data already imported.');
    ...
    protected static function getAvailableDates($accountId, $url)
    {
        try {
            if (!array_key_exists($accountId.$url, self::$availableDates) || defined('PIWIK_TEST_MODE')) {
                $finalDates = StaticContainer::get('Piwik\Plugins\SearchEngineKeywordsPerformance\Client\Google')
                ...
                self::$availableDates[$accountId.$url] = StaticContainer::get('Piwik\Plugins\SearchEngineKeywordsPerformance\Client\Google')
                    ->getDatesWithSearchAnalyticsData($accountId, $url, false);
    ...
    protected function getKeywordsFromConsoleAsDataTable($accountId, $url, $date, $type)
    {
        ...
        try {
            if (!defined('PIWIK_TEST_MODE') && !$this->isImportAllowedForDate($date)) {
                Log::debug("[SearchEngineKeywordsPerformance] Skip fetching keywords from Search Console for today and dates more than 500 days in the past: " . $date);
                return null;
            }
        ...
        if (!self::isFinalDate($accountId, $url, $date)) {
            $dataTable->setMetadata(self::DATATABLE_METADATA_TEMPORARY, true);
        }
        ...
        foreach ($rows as $keywordDataSet) {
            /** @var \Google_Service_SearchConsole_ApiDataRow $keywordDataSet */

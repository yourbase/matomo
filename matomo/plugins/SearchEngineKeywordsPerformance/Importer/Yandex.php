...
class Yandex
{
    ...
    public static function importAvailableDataForDate($accountId, $hostId, $date, $force=false)
    {
        ...
        if ($timestamp > time()) {
            return []; // no import for dates in the future
        }

        if ($timestamp > time()-self::MAX_DAYS_KEYWORD_DATA_DELAY*24*3600) {
            $force = true; // always reimport the last few days
        }
        ...
        try {
            ...
            if ($availableKeywordsDataTable->getRowsCountWithoutSummaryRow() > 0 && !$force) {
                Log::debug("[SearchEngineKeywordsPerformance] Yandex keywords already imported for $hostId and date $date");
            } else {
                Log::debug("[SearchEngineKeywordsPerformance] Fetching Yandex keywords for $hostId and date $date");

                $keywords = StaticContainer::get('Piwik\Plugins\SearchEngineKeywordsPerformance\Client\Yandex')
                    ->getSearchAnalyticsData($accountId, $hostId, $date);
                ...
                if ($dataTable && ($dataTable->getRowsCountWithoutSummaryRow() > 0 || $timestamp < time()-self::MAX_DAYS_KEYWORD_DATA_DELAY*24*3600)) {
                    ...
                    Log::debug("[SearchEngineKeywordsPerformance] Importing Yandex keywords for $hostId / $date");

                    $model->archiveKeywordData($hostId, $date, $keywordData[0]);
                }
            }
        } catch (InvalidCredentialsException $e) {
            Log::info('[SearchEngineKeywordsPerformance] Error while importing Yandex keywords for ' . $hostId . ': ' . $e->getMessage());
        } catch (RateLimitApiException $e) {
            Log::info('[SearchEngineKeywordsPerformance] Error while importing Yandex keywords for ' . $hostId . ': ' . $e->getMessage());
        } catch (\Exception $e) {
            Log::error('[SearchEngineKeywordsPerformance] Error while importing Yandex keywords for ' . $hostId . ': ' . $e->getMessage());
        }
        ...
        try {
            ...
            if (!empty($availableCrawlStats) && !$force) {
                Log::debug("[SearchEngineKeywordsPerformance] Yandex crawl stats already imported for $hostId and date $date");
            } else {
                Log::debug("[SearchEngineKeywordsPerformance] Fetching Yandex crawl stats for $hostId and date $date");

                $crawlStats = StaticContainer::get('Piwik\Plugins\SearchEngineKeywordsPerformance\Client\Yandex')
                     ->getCrawlStats($accountId, $hostId, $date);
                 ...
                 if ($dataTable) {
                     ...
                     Log::debug("[SearchEngineKeywordsPerformance] Importing Yandex crawl stats for $hostId and date $date");

                     $model->archiveCrawlStatsData($hostId, $date, $keywordData[0]);
                 }
             }
        } catch (InvalidCredentialsException $e) {
            Log::info('[SearchEngineKeywordsPerformance] Error while importing Yandex crawl stats for ' . $hostId . ': ' . $e->getMessage());
        } catch (RateLimitApiException $e) {
            Log::info('[SearchEngineKeywordsPerformance] Error while importing Yandex crawl stats for ' . $hostId . ': ' . $e->getMessage());
        } catch (\Exception $e) {
            Log::error('[SearchEngineKeywordsPerformance] Error while importing Yandex crawl stats for ' . $hostId . ': ' . $e->getMessage());
        }
    ...
    protected function completeExistingArchiveIfAny($period)
    {
        ...
        $archiver = new YandexArchiver($archiveProcessor);

...
class Bing
{
    ...
    protected function sendAPIRequest($apiKey, $method, $params = [])
    {
        ...
        while ($retries < 5) {
            ...
            if (!empty($response['ErrorCode'])) {
                ...
                $isThrottledHost = (strpos($response['Message'], 'ThrottleHost') !== false ||
                                   $response['ErrorCode'] == self::API_ERROR_THROTTLE_HOST);
                $isThrottledUser = (strpos($response['Message'], 'ThrottleUser') !== false ||
                                   $response['ErrorCode'] == self::API_ERROR_THROTTLE_USER);
                $isThrottled = $isThrottledHost || $isThrottledUser;
    ...
    public function throwIfThrottled($apiKey)
    {
        ...
        if (Date::now()->subHour(self::THROTTLE_BREAK_HOURS)->isEarlier($throttleDate)) {
            throw new RateLimitApiException('API requests temporarily throttled till ' . $throttleDate->addHour(self::THROTTLE_BREAK_HOURS)->getDatetime(), self::API_ERROR_THROTTLE_USER);
        }
    }
}

...
class Yandex
{
    ...
    {
        return true && $this->getClientConfig();
    }
        ...
        if (property_exists($availableSites, 'hosts')) {
            foreach ($availableSites->hosts as $availableSite) {
                if (!$removeUrlsWithoutAccess || $availableSite->verified) {
                    $sites[$availableSite->unicode_host_url] = [
                        'verified' => $availableSite->verified,
                        'host_id'  => $availableSite->host_id
                    ];
                }
            }
        }
    ...
    public function getSearchAnalyticsData($accountId, $hostId, $date)
    {
        ...
        foreach ($searchQueries->queries as $query) {
            $keywords[] = [
                ...
                'position'    => $query->indicators->AVG_SHOW_POSITION,
            ];
        }
    ...
    public function getCrawlStats($accountId, $hostId, $date)
    {
        ...
        $crawlStats = $this->retryApiMethod(function() use ($accessToken, $hostId, $date){
            return $this->getIndexingHistory($accessToken, $hostId, $date);
        });
        ...
        if (!empty($crawlStats) && !empty($crawlStats->indicators)) {
            ...
            foreach ($indicators as $indicator => $indicatorByDate) {
                foreach ($indicatorByDate as $dateItem) {
                    ...
                    if (empty($crawlStatsByDate[$indicator])) {
                        $crawlStatsByDate[$indicator] = 0;
                    }

                    $crawlStatsByDate[$indicator] += $dateItem->value;
                }
            }
        }

        $pagesInIndex = $this->retryApiMethod(function() use ($accessToken, $hostId, $date){
            return $this->getPagesInIndex($accessToken, $hostId, $date);
        });
        ...
        if (!empty($pagesInIndex) && !empty($pagesInIndex->history)) {
            ...
            foreach ($history as $entry) {
                if (empty($crawlStatsByDate['SEARCHABLE'])) {
                    $crawlStatsByDate['SEARCHABLE'] = 0;
                }

                $crawlStatsByDate['SEARCHABLE'] += $entry->value;
            }
        }

        $pageChanges = $this->retryApiMethod(function() use ($accessToken, $hostId, $date){
            return $this->getPageChangesInSearch($accessToken, $hostId, $date);
        });
    ...
    public function processAuthCode($authCode)
    {
        ...
        Piwik::postEvent('SearchEngineKeywordsPerformance.AccountAdded', [
            [
                'provider' => \Piwik\Plugins\SearchEngineKeywordsPerformance\Provider\Yandex::getInstance()->getName(),
    ...
    protected function fetchAccessTokenWithAuthCode($authCode)
    {
        ...
        $response = Http::sendHttpRequestBy(
            Http::getTransportMethod(), 'https://oauth.yandex.com/token',
            2000, null, null, null, 0,
            false, false, false, false,
            'POST', $clientConfig['id'], $clientConfig['secret'],
    ...
    public function addAccount($id, $accessToken, $username)
    {
        ...
        $config = [
            'userInfo'    => [
                'picture' => 'https://avatars.yandex.net/get-yapic/' . $userInfo['default_avatar_id'] . '/islands-retina-50',
    ...
    protected function getUserInfoByAccessToken($accessToken)
    {
        ...
        $response = Http::sendHttpRequestBy(
            Http::getTransportMethod(), $url, 2000,
            null, null, null, 0,
            false, false, false, false,
            'GET', null, null, null,
    ...
    protected function getPopularQueries($accessToken, $hostId, $date)
    {
        return $this->sendApiRequest(
            ...
            [
                'date_from' => date(DATE_ATOM, $date),
                'date_to' => date(DATE_ATOM, ($date+24*3600-1)),
    ...
    protected function getIndexingHistory($accessToken, $hostId, $date)
    {
        return $this->sendApiRequest(
            ...
            array('date_from' => date(DATE_ATOM, $date), 'date_to' => date(DATE_ATOM, ($date+24*3600-1)))
        );
    }
    ...
    protected function sendApiRequest($accessToken, $method, $params = [])
    {
        ...
        if (empty($response['data'])) {
            throw new \Exception('Yandex API returned no data: ' . var_export($response, true));
        }
